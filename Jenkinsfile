    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=LTS -v ${TEMPDIR}:/ansible/readme linuxserver/doc-builder:latest
              if [ "$(md5sum ${TEMPDIR}/${LS_REPO}/Jenkinsfile | awk '{ print $1 }')" != "$(md5sum Jenkinsfile | awk '{ print $1 }')" ] || \
                ...
                git --git-dir ${TEMPDIR}/repo/${LS_REPO}/.git checkout -f LTS
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-LTS"
          sh "docker tag ${IMAGE}:arm32v7-${META_TAG} ${IMAGE}:arm32v7-LTS"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-LTS"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v7-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-LTS"
          sh "docker push ${IMAGE}:arm32v7-LTS"
          sh "docker push ${IMAGE}:arm64v8-LTS"
          sh "docker manifest push --purge ${IMAGE}:LTS || :"
          sh "docker manifest create ${IMAGE}:LTS ${IMAGE}:amd64-LTS ${IMAGE}:arm32v7-LTS ${IMAGE}:arm64v8-LTS"
          sh "docker manifest annotate ${IMAGE}:LTS ${IMAGE}:arm32v7-LTS --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:LTS ${IMAGE}:arm64v8-LTS --os linux --arch arm64 --variant v8"
