      ...
      steps {
        withCredentials([
          ...
          sh '''#! /bin/bash
             echo $DOCKERPASS | docker login -u $DOCKERUSER --password-stdin
             '''
          sh "docker tag ${IMAGE}:${META_TAG} ${IMAGE}:bionic"
          sh "docker push ${IMAGE}:bionic"
    ...
    stage('Docker-Push-Multi') {
      ...
      steps {
        withCredentials([
          ...
          sh "docker tag ${IMAGE}:amd64-${META_TAG} ${IMAGE}:amd64-bionic"
          sh "docker tag ${IMAGE}:arm32v6-${META_TAG} ${IMAGE}:arm32v6-bionic"
          sh "docker tag ${IMAGE}:arm64v8-${META_TAG} ${IMAGE}:arm64v8-bionic"
          sh "docker push ${IMAGE}:amd64-${META_TAG}"
          sh "docker push ${IMAGE}:arm32v6-${META_TAG}"
          sh "docker push ${IMAGE}:arm64v8-${META_TAG}"
          sh "docker push ${IMAGE}:amd64-bionic"
          sh "docker push ${IMAGE}:arm32v6-bionic"
          sh "docker push ${IMAGE}:arm64v8-bionic"
          sh "docker manifest push --purge ${IMAGE}:bionic || :"
          sh "docker manifest create ${IMAGE}:bionic ${IMAGE}:amd64-bionic ${IMAGE}:arm32v6-bionic ${IMAGE}:arm64v8-bionic"
          sh "docker manifest annotate ${IMAGE}:bionic ${IMAGE}:arm32v6-bionic --os linux --arch arm"
          sh "docker manifest annotate ${IMAGE}:bionic ${IMAGE}:arm64v8-bionic --os linux --arch arm64 --variant v8"
